...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans">
      ...
      <div className="bg-gov-green-dark text-white text-xs">
        <div className="w-full px-4 sm:px-8 lg:px-16 py-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="text-sm leading-snug flex items-center gap-3">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-green-200">Ministry of Social Justice and Empowerment, Govt. of India</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-3 text-[11px]">
            <div className="flex items-center gap-1 hover:text-green-200 cursor-pointer">
            ...
            <div className="flex items-center gap-2 text-white/80">
              <Facebook size={14} className="hover:text-green-200 cursor-pointer" />
              <Twitter size={14} className="hover:text-green-200 cursor-pointer" />
              <Youtube size={14} className="hover:text-green-200 cursor-pointer" />
            </div>
            <div className="flex items-center gap-2">
              <Link to="/login" className="border border-white/60 px-3 py-1 rounded-full hover:bg-white hover:text-gov-green-dark transition-colors">Login</Link>
              <button className="border border-white/60 px-3 py-1 rounded-full hover:bg-white hover:text-gov-green-dark transition-colors">Demo Login</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <header className="bg-white border-b border-gray-200">
        <div className="w-full px-4 sm:px-8 lg:px-16 py-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-6">
            <img src={PmagyLogo} alt="PMAGY" className="w-28 h-28 object-contain" />
            <div className="text-left">
              <p className="text-2xl font-semibold text-gov-green-800 tracking-wide">Adarsh Gram Under PM-AJAY</p>
              <p className="text-xl text-gray-700">Department of Social Justice &amp; Empowerment</p>
              <p className="text-lg text-gray-600">Government of India</p>
            </div>
          </div>
      ...
      <nav className="bg-gov-green-700 text-white shadow border-b-4 border-gov-saffron sticky top-0 z-40">
          ...
          <div className="hidden md:flex items-stretch text-sm font-semibold">
            {[
              ...
              <Link
                key={item.label}
                to={item.path}
                className="px-5 py-4 border-r border-white/10 hover:bg-gov-green-600 relative transition-colors"
              >
                <span>{item.label}</span>
                {item.badge && (
                  <span className="absolute -top-1 right-3 text-[9px] bg-gov-saffron text-white px-1.5 py-0.5 rounded-sm">
                    {item.badge}
                  </span>
                )}
              </Link>
        ...
        {mobileNavOpen && (
          <div className="md:hidden border-t border-gov-green-600 bg-gov-green-800 px-4 py-3 space-y-2 text-sm">
            {[
              { label: 'Home', path: '/' },
              { label: 'About Scheme', path: '/about' },
              { label: 'Reports', path: '/reports' },
              ...
              <Link key={item.label} to={item.path} className="block py-2 hover:text-green-200" onClick={() => setMobileNavOpen(false)}>
                {item.label}
              </Link>
            ))}
          </div>
      ...
      <main id="main-content" tabIndex={-1} className="outline-none">
        <section className="w-full px-4 sm:px-8 lg:px-16 mt-8 grid gap-6 lg:grid-cols-[2fr,1fr]">
            ...
            <div className="absolute inset-0 flex animate-[heroSlide_24s_linear_infinite]">
              {mockData.heroImages.concat(mockData.heroImages[0]).map((image, index) => (
                <div
                  ...
                  style={{ backgroundImage: `linear-gradient(90deg, rgba(20, 83, 45, 0.85), rgba(20, 83, 45, 0.55)), url(${image.url})` }}
                >
                  <div className="h-full w-full flex flex-col justify-end p-8 text-white">
                    <p className="text-sm uppercase tracking-[0.3em] text-gov-saffron">PM-AJAY Spotlight</p>
                    <h2 className="text-3xl font-semibold max-w-xl">{image.caption}</h2>
                    <button className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-gov-saffron hover:text-white transition-colors">
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col min-h-[24rem]">
            <div className="bg-gov-green-600 text-white px-6 py-4 font-semibold tracking-wide">
              New &amp; Updates
            </div>
                ...
                <div className="flex flex-col gap-4 animate-[ticker_16s_linear_infinite]">
                  {tickerItems.map((item, idx) => (
                    <div key={`${item.title}-${idx}`} className="border-b border-gray-100 pb-3">
                      <p className="text-sm font-semibold text-gov-green-800">{item.title}</p>
        ...
        <section className="mt-12 bg-gray-50 border-y border-gray-100 py-12">
          <div className="w-full px-4 sm:px-8 lg:px-16">
            <div className="flex flex-col gap-3 text-center mb-10">
              <p className="text-sm uppercase tracking-[0.4em] text-gov-green-600">Mission Dashboard</p>
      ...
      <footer className="bg-gov-green-800 text-white mt-12">
        ...
        <div className="bg-gov-green-900 text-center text-xs text-green-200 py-3">
